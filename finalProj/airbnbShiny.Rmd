...
Predict the nightly price of an Airbnb listing that has the chosen features below. This algorithm uses multivariate linear regression and was trained on real Seattle Airbnb listings. 
```{r inputs}
...
renderValueBox({
  ...
  userPred = predict(lmMod, userInput)
  prediction = paste('$', ceiling(userPred))
...
### 95% prediction interval (95% of Airbnb listings with the given featuers have a price that falls in this range) {.value-box}
```{r}
renderValueBox({
  host_is_superhost = input$host
  room_type = input$room
  ...
  pi = paste(lwr, ceiling(outcome$upr), sep = '-')
...
### Reviews per month
```{r}
plot_ly(data = air, x = ~reviews_per_month, y = ~price, color = ~host_is_superhost) %>% layout(xaxis = list(title = "Reviews per month"),
         yaxis = list (title = "Listing price ($)")) 
```
